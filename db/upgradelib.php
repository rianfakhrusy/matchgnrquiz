...
/**
 * Upgrade library code for the matchgnrquiz question type.
 *
...
/**
 * Class for converting attempt data for matchgnrquiz questions when upgrading
 ...
 * @license   http://www.gnu.org/copyleft/gpl.html GNU GPL v3 or later
 */
class qtype_matchgnrquiz_qe2_attempt_updater extends question_qtype_attempt_updater {
    protected $stems;
    protected $choices;
    protected $right;
    protected $stemorder;
    ...
    public function question_summary() {
        ...
        foreach ($this->question->options->subquestions as $matchgnrquizsub) {
            $ans = $matchgnrquizsub->answertext;
            $key = array_search($matchgnrquizsub->answertext, $this->choices);
            if ($key === false) {
                $key = $matchgnrquizsub->id;
                $this->choices[$key] = $matchgnrquizsub->answertext;
            }

            if ($matchgnrquizsub->questiontext !== '') {
                $this->stems[$matchgnrquizsub->id] = $this->to_text($matchgnrquizsub->questiontext);
                $this->right[$matchgnrquizsub->id] = $key;
            }
        }
    ...
    protected function lookup_choice($choice) {
        foreach ($this->question->options->subquestions as $matchgnrquizsub) {
            if ($matchgnrquizsub->id == $choice) {
                if (array_key_exists($matchgnrquizsub->id, $this->choices)) {
                    return $matchgnrquizsub->id;
                } else {
                    return array_search($matchgnrquizsub->answertext, $this->choices);
                }
            }
        }
        return null;
            ...
            if (array_key_exists($stemid, $this->stems) && $choices[$stemid]) {
                ...
                if ($choiceid) {
                    ...
                    $this->logger->log_assumption("Dealing with a place where the
                            ...
                            matchgnrquiz question {$this->question->id}");
    ...
    public function supply_missing_first_step_data(&$data) {
        throw new coding_exception('qtype_matchgnrquiz_updater::supply_missing_first_step_data ' .
                'not tested');
