...
/**
 * Represents a matchgnrquizing question.
 *
 * @copyright 2009 The Open University
 * @license   http://www.gnu.org/copyleft/gpl.html GNU GPL v3 or later
 */
class qtype_matchgnrquiz_question extends question_graded_automatically_with_countback {
    ...
    public $stems;
    /** @var array of choices that can be matchgnrquized to each stem. */
    public $choices;
    ...
    public function apply_attempt_state(question_attempt_step $step) {
        ...
        foreach ($this->stemorder as $stemid) {
            if (!isset($this->stems[$stemid])) {
                $this->stems[$stemid] = html_writer::span(
                        get_string('deletedsubquestion', 'qtype_matchgnrquiz'), 'notifyproblem');
        ...
        foreach ($this->choiceorder as $choiceid) {
            if (!isset($this->choices[$choiceid])) {
                $this->choices[$choiceid] = get_string('deletedchoice', 'qtype_matchgnrquiz');
            }
        }
    }
    ...
    public function classify_response(array $response) {
        ...
        foreach ($this->stems as $stemid => $stem) {
            ...
            if ($choice == get_string('deletedchoice', 'qtype_matchgnrquiz')) {
    ...
    public function prepare_simulated_post_data($simulatedresponse) {
        ...
        foreach ($simulatedresponse as $stem => $choice) {
            ...
            if (empty($choice)) {
                ...
                throw new coding_exception("Unknown choice {$choice} in matchgnrquizing question - {$this->name}.");
            }
    ...
    public function get_validation_error(array $response) {
        ...
        return get_string('pleaseananswerallparts', 'qtype_matchgnrquiz');
    }
    ...
    public function check_file_access($qa, $options, $component, $filearea, $args, $forcedownload) {
        if ($component == 'qtype_matchgnrquiz' && $filearea == 'subquestion') {
